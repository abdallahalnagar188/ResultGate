...
import eramo.resultgate.databinding.FragmentQueryRepliedBinding
...
import eramo.resultgate.presentation.viewmodel.drawer.myaccount.queries.QueryRepliedViewModel
...
@AndroidEntryPoint
class QueryRepliedFragment : Fragment(R.layout.fragment_query_replied), QueryAdapter.OnItemClickListener {

    @Inject
    lateinit var queryAdapter: QueryAdapter
    private val viewModel by viewModels<QueryRepliedViewModel>()
    private lateinit var binding: FragmentQueryRepliedBinding
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding = FragmentQueryRepliedBinding.bind(view)

        queryAdapter.setListener(this@QueryRepliedFragment)

        binding.apply {
            FQRepliedRv.adapter = queryAdapter
        }
    ...
    override fun onResume() {
        super.onResume()
        viewModel.getRepliedRequests()
    }

    private fun fetchGetRequestsState() {
        lifecycleScope.launchWhenCreated {
            viewModel.repliedRequestsDtoState.collect { state ->
                when (state) {
                    is UiState.Success -> {
