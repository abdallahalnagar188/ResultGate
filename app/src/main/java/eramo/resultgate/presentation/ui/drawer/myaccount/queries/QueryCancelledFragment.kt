...
import eramo.resultgate.databinding.FragmentQueryCancelledBinding
...
import eramo.resultgate.presentation.viewmodel.drawer.myaccount.queries.QueryCancelledViewModel
...
@AndroidEntryPoint
class QueryCancelledFragment : Fragment(R.layout.fragment_query_cancelled),QueryAdapter.OnItemClickListener {

    @Inject
    lateinit var queryAdapter: QueryAdapter
    private val viewModel by viewModels<QueryCancelledViewModel>()
    private lateinit var binding: FragmentQueryCancelledBinding
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding = FragmentQueryCancelledBinding.bind(view)

        queryAdapter.setListener(this@QueryCancelledFragment)
        binding.apply {
            FQCancelledRv.adapter = queryAdapter
        }
    ...
    override fun onResume() {
        super.onResume()
        viewModel.getCancelledRequests()
    }

    private fun fetchGetRequestsState() {
        lifecycleScope.launchWhenCreated {
            viewModel.cancelledRequestsDtoState.collect { state ->
                when (state) {
                    is UiState.Success -> {
